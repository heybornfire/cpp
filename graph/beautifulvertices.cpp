#include<bits/stdc++.h>
#include <ext/pb_ds/assoc_container.hpp>
using namespace __gnu_pbds;
using namespace std;

#define ff              first
#define ss              second
#define int             long long
#define pb              push_back
#define mp              make_pair
#define pii             pair<int,int>
#define vi              vector<int>
#define mii             map<int,int>
#define pqb             priority_queue<int>
#define pqs             priority_queue<int,vi,greater<int> >
#define setbits(x)      __builtin_popcountll(x)
#define zrobits(x)      __builtin_ctzll(x)
#define mod             1000000007
#define inf             1e18
#define ps(x,y)         fixed<<setprecision(y)<<x
#define mk(arr,n,type)  type *arr=new type[n];
#define w(x)            int x; cin>>x; while(x--)
mt19937                 rng(chrono::steady_clock::now().time_since_epoch().count());

typedef tree<int, null_type, less<int>, rb_tree_tag, tree_order_statistics_node_update> pbds;


void c_p_c()
{
	ios_base::sync_with_stdio(0); cin.tie(0); cout.tie(0);
#ifndef ONLINE_JUDGE
	freopen("/home/krishna/Documents/competitive/input.txt", "r", stdin);
	freopen("/home/krishna/Documents/competitive/output.txt", "w", stdout);
#endif
}
int cnt=0;
void bfs(list<int>adjlist[],int node,bool visited[],int children[],int parent[])
{	
	//cout<<node<<endl;
	visited[node]=true;
	for(int i:adjlist[node])
	{
				

		
		if(!visited[i])
			{
				children[node]++;
				parent[i]=node;
				bfs(adjlist,i,visited,children,parent);
			}
	}
}
int32_t main()
{
	c_p_c();
	int vertices,edges,u,v;
	cin>>vertices>>edges;
	list<int> adjlist[vertices+1];
	for(int i=0;i<edges;i++)
	{
		cin>>u>>v;
		adjlist[u].push_back(v);
		adjlist[v].push_back(u);
	}
	bool visited[vertices+1]={false};
	int parent[vertices+1]={0};
	int children[vertices+1]={0};

	for(int i=1;i<=vertices;i++)
	{
		if(!visited[i])
		{	parent[i]=-1;
			//cout<<i<<endl;
			bfs(adjlist,i,visited,children,parent);
		}
	}
	for(int i=1;i<=vertices;i++)
	{
		if(parent[i]!=-1)
		{
			if(children[i]>children[parent[i]])
				cnt++;
		}
	}
	cout<<cnt<<endl;
	// for(int i=1;i<=vertices;i++)
	// {cout<<children[i]<<endl;}
	return 0;
}
