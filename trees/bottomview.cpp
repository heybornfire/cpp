...
void bottomview(node *root,int level,int distance,map<int,pair<int,int>> &mp)
{
	if(!root)
		return;
	if(mp.find(distance)==mp.end())
	{
		mp.insert({distance,{level,root->data}});
	}
	else
	{
		if(mp[distance].first<level)
	...
	bottomview(root->right,level+1,distance+1,mp);
	bottomview(root->left,level+1,distance-1,mp);
}
int32_t main()
{
	c_p_c();
